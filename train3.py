...
def model_fn(features, labels, mode, params):
    ...
    decaying_step = tf.train.exponential_decay(params["learning_rate"], global_step, 10000, 0.96, staircase = True)
    tf.summary.scalar('learning_rate', decaying_step)

    optimiser = tf.train.MomentumOptimizer(
        learning_rate=decaying_step,
        momentum=0.9)
...
def train(args):
    ...
    train_filenames = all_filenames[:513]
    val_filenames = all_filenames[513:]
